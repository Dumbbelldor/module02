...
final class SqlQueries {
    ...
    static final String GIFT_FIND_BY_ID = "select * from gift_certificate" +
            " where id = ?";

    static final String GIFT_FIND_BY_NAME = "select * from gift_certificate" +
            " where name = ?";

    static final String GIFT_FIND_ALL = "select * from gift_certificate";

    static final String GIFT_UPDATE = "update gift_certificate" +
            " set name = ?, description = ?, price = ?, duration = ?," +
            " create_date = ?, last_update_time = ? where id = ?";

    static final String GIFT_SAVE = "insert into gift_certificate" +
            " (id, name, description, price, duration, create_date," +
            " last_update_time) values (?, ?, ?, ?, ?, ?, ?)";

    static final String GIFT_FIND_BY_TAG_ID = "select gc.* from" +
            " gift_tag_mapping gtm" +
            " inner join gift_certificate gc on gc.id = gtm.gc_id" +
            " inner join tags t on t.id = gtm.t_id" +
            " where t.id = ?";

    static final String GIFT_FIND_BY_TAG_NAME = "select gc.*" +
            " from gift_tag_mapping gtm" +
            " inner join gift_certificate gc on gc.id = gtm.gc_id" +
            " inner join tags t on t.id = gtm.t_id" +
            " where t.name = ?";

    static final String GIFT_DELETE_BY_ID = "delete from gift_certificate" +
            " where id = ?";

    static final String GIFT_FIND_BY_PARTIAL_NAME = "select *" +
            " from gift_certificate gc" +
            " where lower(gc.name) like ?";

    static final String GIFT_FIND_BY_PARTIAL_DESC = "select *" +
            " from gift_certificate gc" +
